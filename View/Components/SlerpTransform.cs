...
{
    public class SlerpTransform : AbstractLerper
    {
        public override IEnumerator Animate(object target, object from, object to, float time, params Action[] callbacks)
        {
            ...
            while (elapsed < time)
            {
                ...
                ((Transform)target).rotation = Quaternion.Slerp(fromRotation, toRotation, t);
                ((Transform)target).position = Vector3.Slerp(fromPosition, toPosition, t);
                ((Transform)target).localScale = Vector3.Slerp(fromScale, toScale, t);
